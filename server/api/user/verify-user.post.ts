...
export default defineEventHandler(async (event) => {
	try {
		...
		const user = await userModel
			.findOne({
				uuid: body.uuid,
				verificationCodeExpAt: { $gt: new Date(Date.now()) },
			})
			.select("+verificationCode +verificationCodeExpAt");
		if (!user) {
			return createError({
				statusCode: 404,
				statusMessage:
		...
		const isVerificationCodeCorrect = await compareStrings(
