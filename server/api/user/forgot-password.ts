	...
	try {
		...
		if (!emailTemplate) {
			return createError({
				statusCode: 500,
				statusMessage:
					"Apologies, an internal server error while sending verification email. Please contact development team.",
			});
		}
		...
		const sendAnEmail = await sendEmail({
			body: emailTemplate,
			email: [user.email],
			mergeFields: { url: URLLink },
			from: `auth.com.ge <${useRuntimeConfig().EMAIL}>`,
			replyTo: `auth.com.ge <${useRuntimeConfig().EMAIL}>`,
